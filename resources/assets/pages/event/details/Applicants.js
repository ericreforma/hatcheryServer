...
export default class Applicants extends Component {
    constructor(props){
        ...
        this.state = {
            ...
            applicantTabs: ['Pending','Accepted','Completed', 'Rejected'],
            profileTab:['Profile','Application History', 'Reviews'],
            activeProfileTab: 0,
            activeApplicantTab: 0,
            applicant: null,
            profile: null,
            pendingCount: this.props.event.applicants.filter(a => (a.status == 'REQUESTED' || a.status == 'PENDING')).length,
            acceptedCount: this.props.event.applicants.filter(a => a.status == 'ACCEPTED').length,
            completedCount: this.props.event.applicants.filter(a => a.status == 'COMPLETED').length,
    ...
    requestedList = () => 
        ...
        this.state.event.applicants.filter(a => (a.status == 'REQUESTED' || a.status == 'PENDING')).map((applicant, index) => 
    ...
    completedList = () => 
        this.state.event.applicants && 
        this.state.event.applicants.filter(a => a.status == 'COMPLETED').map((applicant, index) => 
            this.applicantItem(applicant, index)
        )
    ...
    refresh = activeApplicantTab => {
        ...
        .then(response => {
            this.setState({
                event: response.data,
                pendingCount:  response.data.applicants.filter(a => (a.status == 'REQUESTED' || a.status == 'PENDING')).length,
                acceptedCount:  response.data.applicants.filter(a => a.status == 'ACCEPTED').length,
                completedCount:  response.data.applicants.filter(a => a.status == 'COMPLETED').length,
                    ...
                    <div className='custom_tab'>
                        ...
                        <div className='custom_list'>
                            { this.state.activeApplicantTab === 0 ? 
                                ...
                                this.acceptedList() :
                                this.state.activeApplicantTab === 2 ? 
                                this.completedList() :
                                this.rejectedList()
                            }
                        </div>
